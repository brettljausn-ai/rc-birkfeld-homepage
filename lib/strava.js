...
async function getClubActivities() {
  try {
    const cached = await getCached('activities');
    if (cached) return cached;

    const { STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET, STRAVA_REFRESH_TOKEN } = process.env;
    if (!STRAVA_CLIENT_ID || !STRAVA_CLIENT_SECRET || !STRAVA_REFRESH_TOKEN) return [];
    ...
    await setCache('activities', arr);
    return arr;
  } catch (err) {
    console.error('Strava-Aktivitäten-Fehler:', err.message);
    return [];
  }
}

module.exports = { getStravaData, getClubActivities };
